...
def playback_dataset(args):
    ...
    if args.demo_key is not None:
        if args.demo_key not in demos:
            raise ValueError(
                "demo key {} not found in {}".format(args.demo_key, args.dataset)
            )
        demos = [args.demo_key]
    ...
    for ind in range(len(demos)):
            ...
            results.append(
                {
                    ...
                    "reason": "use_obs playback does not instantiate the environment",
                }
            )
            continue
        ...
        success = playback_trajectory_with_env(
            env=env,
        ...
        results.append(
            {
                ...
                "trajectory_length": int(states.shape[0]),
                "used_actions": bool(args.use_actions),
            }
        )
    ...
    result = {
        ...
        "video_path": os.path.abspath(args.video_path) if write_video else None,
        "used_actions": bool(args.use_actions),
        "demos": results,
        "success": bool(any(item.get("success") for item in results)),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="(optional) exact demonstration key to play, for example demo_42",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="(optional) write structured playback result metadata to this JSON path",
    )
